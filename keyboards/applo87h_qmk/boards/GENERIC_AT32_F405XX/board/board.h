...
/*
 * Setup for a Generic AT32F405 board (SLK Applo87H).
 */
 ...
 * Schematic uses a 12 MHz HEXT crystal.
 */
 ...
 * Defines AT32F402_405Rx and AT32F402_405xC in at32_registry.h,
 * enabling GPIOA/B/C/D/F and OTG2 (OTGHS).
 */
#define AT32F405RC
 ...
 *
 * Despite the legacy name "ULPI", on AT32F405 this macro tells the
 * OTGv1 USB LLD to:
 *   - call crmEnableOTG_HS() (OTGHS clock path)
 *   - NOT set GUSBCFG.PHYSEL (select on-chip HS PHY, not FS transceiver)
 *   - set DCFG.DEVSPD = HS (480 Mbps) when AT32_USE_USB_OTG2_HS=TRUE
 *
 * Without this macro OTG2 is forced to FS11 mode (12 Mbps) and 8K Hz
...
/*
 * Port A setup: all inputs with pull-up (PA13/PA14 SWD are handled by hardware).
 */
...
/*
 * Port B setup: all inputs with pull-up.
 */
...
/*
 * Port C setup: all inputs with pull-up.
 */
...
/*
 * Port D setup.
 * PD0  - HEXT IN  (floating/analog, used by crystal)
 * PD1  - HEXT OUT (floating/analog, used by crystal)
 * PD2  - Scroll Lock LED (matrix will reconfigure as needed)
 */
...
/*
 * Port F setup: all inputs with pull-up.
 * PF0  - Reset button
 * PF11 - Boot button
 */
